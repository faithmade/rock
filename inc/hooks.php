...
add_action( 'rock_header', 'rock_add_site_title' );
...
add_action( 'rock_header', 'rock_add_hero' );
 ...
 * @since  1.0.0
 */
function rock_add_hero_content() {
...
add_action( 'rock_hero', 'rock_add_hero_content' );

/**
 * Display mobile menu html.
 *
 * @action rock_before_site_navigation
 * @since  1.0.0
 */
function rock_add_mobile_menu() {

	get_template_part( 'templates/parts/mobile-menu' );

}
add_action( 'rock_before_site_navigation', 'rock_add_mobile_menu' );
...
function rock_add_primary_menu() {
	...
	wp_nav_menu(
		array(
			...
			'walker'         => new Rock_Walker_Nav_Menu,
		)
	);

}
add_action( 'rock_site_navigation', 'rock_add_primary_menu' );
...
add_action( 'rock_after_header', 'rock_add_primary_navigation' );
...
add_action( 'rock_after_header', 'rock_add_page_title' );
...
add_action( 'rock_footer', 'rock_add_footer_widgets' );

/**
 * Display site info after the footer.
 *
 * @action rock_after_footer
 * @since  1.0.0
 */
...
add_action( 'rock_after_footer', 'rock_add_site_info' );

/**
 * Display footer navigation menu in the footer.
 *
 * @action rock_site_info
 * @since  1.0.0
 */
function rock_add_footer_navigation() {
...
add_action( 'rock_site_info', 'rock_add_footer_navigation', 5 );
 ...
 * @since  1.0.0
 */
function rock_add_social_navigation() {
...
add_action( 'rock_site_info', 'rock_add_social_navigation', 7 );
 ...
 * @since  1.0.0
 */
function rock_add_credit() {

	get_template_part( 'templates/parts/credit' );

}
add_action( 'rock_site_info', 'rock_add_credit' );
 ...
 *
 * remove_filter( 'excerpt_length', 'rock_excerpt_length' );
 ...
 * @return int
 */
function rock_excerpt_length( $length ) {

	return 20;

}
add_filter( 'excerpt_length', 'rock_excerpt_length' );
 ...
 *
 * remove_filter( 'excerpt_more', 'rock_excerpt_more' );
 ...
 * @return string
 */
function rock_excerpt_more( $more ) {

	return ! is_admin() ? '&hellip;' : $more;

}
add_filter( 'excerpt_more', 'rock_excerpt_more' );
 ...
 *
 * remove_filter( 'script_loader_tag', 'rock_conditional_jquery_tag', 10, 2 );
 *
 ...
 * @return string
 */
function rock_conditional_jquery_tag( $tag, $handle ) {

	return ( 'jquery' === $handle ) ? "<!--[if (gte IE 9) | (!IE)]><!-->$tag<!--<![endif]-->" : $tag;

}
add_filter( 'script_loader_tag', 'rock_conditional_jquery_tag', 10, 2 );
...
add_filter( 'body_class', 'rock_body_class' );
...
function rock_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		...
		$title .= sprintf(
			...
			$sep, // xss ok
			sprintf(
				esc_html_x( 'Page %d', 'page number', 'rock' ),
				max( $paged, $page )
			)
		);
