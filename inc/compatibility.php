...
add_action( 'admin_notices', 'ctc_plugin_notice', 9 ); // higher priority than notices coming from plugin (theme notice before plugin addon notices)
...
function ctc_enqueue_ie_unsupported() {
	...
	$support = get_theme_support( 'ctc-ie-unsupported' );
	if ( $support ) {
...
add_action( 'wp_enqueue_scripts', 'ctc_enqueue_ie_unsupported', 1 ); // before all others
