 ...
 * @since  1.0.0
 */
function rock_wc_setup() {

	add_theme_support( 'woocommerce' );

}
add_action( 'after_setup_theme', 'rock_wc_setup' );
 ...
 * @since  1.0.0
 */
function rock_wc_wrapper_start() {
...
add_action( 'woocommerce_before_main_content', 'rock_wc_wrapper_start' );
...
add_action( 'woocommerce_after_main_content', 'rock_wc_wrapper_end' );
 ...
 * @since  1.0.0
 */
function rock_wc_sidebar() {

	get_sidebar( 'tertiary' );

}
add_action( 'woocommerce_sidebar', 'rock_wc_sidebar' );

/**
 * Display WooCommerce messages above post and page content.
 *
 * @action rock_before_post_content
 * @action rock_before_page_content
 * @since  1.0.0
 */
function rock_wc_shop_messages() {
...
add_action( 'rock_before_post_content', 'rock_wc_shop_messages' );
add_action( 'rock_before_page_content', 'rock_wc_shop_messages' );
 ...
 * @return string
 */
function rock_wc_shop_layout( $layout ) {

	if ( is_shop() ) {

		remove_filter( 'rock_current_layout', __FUNCTION__ ); // Prevent infinite loop
...
add_filter( 'rock_current_layout', 'rock_wc_shop_layout' );

/**
 * Filter the WooCommerce shop page title.
 *
 * @filter rock_the_page_title
 * @since  1.0.0
 *
 * @param  string $title
 *
 * @return string
 */
function rock_wc_shop_title( $title ) {
...
add_filter( 'rock_the_page_title', 'rock_wc_shop_title' );
...
add_filter( 'loop_shop_columns',                    'rock_wc_shop_columns' );
add_filter( 'woocommerce_related_products_columns', 'rock_wc_shop_columns' );
add_filter( 'woocommerce_upsells_products_columns', 'rock_wc_shop_columns' );

/**
 * Filter the WooCommerce product class.
 *
 * @filter rock_wc_product_classes
 * @global WP_Post $post
 ...
 * @since 1.0.0
 */
function rock_wc_product_classes( $classes ) {
...
add_filter( 'rock_colors', 'rock_wc_colors' );
